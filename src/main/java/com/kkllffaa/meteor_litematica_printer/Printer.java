...
public class Printer extends Module {
					...
					if (airPlace.get()
							...
							|| !airPlace.get()
									...
									&& MyUtils.getVisiblePlaceSide(
											...
											wantedSlabType,
											wantedBlockHalf,
											wantedHorizontalOrientation != null ? wantedHorizontalOrientation
			...
			BlockIterator.after(() -> {
				...
				for (BlockPos pos : toSort) {
					...
					if (dirtgrass.get() && item == Items.GRASS_BLOCK)
						item = Items.DIRT;
					if (switchItem(item, state, placed == 0, () -> place(state, pos))) {
						timer = 0;
						placed++;
						if (renderBlocks.get()) {
	...
	public boolean place(BlockState required, BlockPos pos) {
		...
		Direction wantedHopperOrientation = advanced.get() && required.contains(Properties.HOPPER_FACING)
				? required.get(Properties.HOPPER_FACING)
				: null;
		// Direction wantedFace = advanced.get() && required.contains(Properties.FACING)
		// ? required.get(Properties.FACING) : null;

		Direction placeSide = placeThroughWall.get() ? MyUtils.getPlaceSide(
				pos,
				required,
				wantedSlabType,
				wantedBlockHalf,
	...
	private boolean switchItem(Item item, BlockState state, boolean canSwitchSlot, Supplier<Boolean> action) {
		if (mc.player == null)
			return false;
	...
	private void onRender(Render3DEvent event) {
		placed_fade.forEach(s -> {
			Color a = new Color(colour.get().r, colour.get().g, colour.get().b,
					(int) (((float) s.getLeft() / (float) fadeTime.get()) * colour.get().a));
